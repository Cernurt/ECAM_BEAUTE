/*
Programme qui pilote le comportement
...
selon un nombre de ticks comptés (voir fonction isrt())
*/
/***********************************************************************************
 * 
...
// variables utiles
volatile double dt = CADENCE_MS / 1000.; // = 0.01 seconde si CADENCE_MS=10
volatile double temps = -CADENCE_MS / 1000.; // temps qui passe
volatile double etat = 0; // état de la trajectoire
...
void isrt() {
  ...
  // cumul des ticks comptés
  cumulCodeurDroit += ticksCodeurDroit;
  ticksCodeurDroit=0;
  cumulCodeurGauche += ticksCodeurGauche;
  ticksCodeurGauche=0;
  // appel des fonctions de commande selon un timing simple
  // pendant 3000 ticks je vais tout droit (même nombre de ticks sur les deux roues)
  // ensuite pendant 1000 ticks sur moteur gauche je tourne
  // ensuite je stop
  if(etat == 0) { // en avant toute
    commandeDroit = 255;
    commandeGauche = 255;
  ...
  else if(etat == 1 && cumulCodeurDroit > 3000 && cumulCodeurGauche > 3000) { // on tourne
    cumulCodeurDroit=0;
    cumulCodeurGauche=0;
    commandeDroit = 0;
    commandeGauche = 255; // comptege des tocks en positif normalement
    CommandeMoteurDroit(commandeDroit);
    CommandeMoteurGauche(commandeGauche);
    etat = 2;
  }
  else if(etat == 2 && cumulCodeurGauche > 1000) { // on stop
    commandeDroit = 0;
    commandeGauche = 0;
    CommandeMoteurDroit(commandeDroit);
    CommandeMoteurGauche(commandeGauche);
