...
void parcoursDE(){
  ...
  distempR = MDROIT.ttlTicks;
  distempL = MGAUCHE.ttlTicks;
  while(((MDROIT.ttlTicks + MGAUCHE.ttlTicks)/2 - (distempR + distempL)/2) < MDROIT.MMtoTicks(50)){
  ...
  if (not borne){
    while(((MDROIT.ttlTicks + MGAUCHE.ttlTicks)/2 - (distempR + distempL)/2) < MDROIT.MMtoTicks(100)){
      ToutDroitCapitaine(0.75, MDROIT.ttlTicks - distempR, MGAUCHE.ttlTicks - distempL);
      bornemuch(3);
      MDROIT.motor.run(FORWARD);   
      MGAUCHE.motor.run(FORWARD);
