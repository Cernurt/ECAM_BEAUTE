...
void avanceDeDecaleInterLigne(float vitesseDem){

  MDROIT.motor.run(FORWARD);     
  MGAUCHE.motor.run(FORWARD);
  positionLigne = qtr.readLineBlack(sensorValues);

  while(sensorValues[0] + sensorValues[1] + sensorValues[2] + sensorValues[3] + sensorValues[4] + sensorValues[5] + sensorValues[6] + sensorValues[7] < 6000){ // Mettre la sensibilité en variable ?
    DecaleDligne(vitesseDem);
    MDROIT.motor.run(FORWARD);
    MGAUCHE.motor.run(FORWARD);
  }
  ToutDroitCapitaine(0,0,0);
...
void avanceDeDecaleLigneLumiere(float distDem, float vitesseDem, float sensi){
  ...
  while(((capteurLumiere() < (SommeLumi/tempi) * sensi) and (MDROIT.ttlTicks - distempR) < MDROIT.MMtoTicks(distDem))){      
    DecaleDligne(vitesseDem);
    MDROIT.motor.run(FORWARD);
    MGAUCHE.motor.run(FORWARD);

    SommeLumi += capteurLumiere();
    tempi+=1;
  }
  if (((MDROIT.ttlTicks + MGAUCHE.ttlTicks)/2 - (distempR + distempL)/2) < MDROIT.MMtoTicks(distDem)){
    ...
    avanceDeMmDecaleLigne(((distDem - MDROIT.ttlTicks + MGAUCHE.ttlTicks)/2 - (distempR + distempL)/2), vitesseDem);
...
void avanceDeMmDecaleLigne(float distDem, float vitesseDem){
...
void avanceDeMmInterLigne(float vitesseDem){
  ...
  while(sensorValues[0] + sensorValues[1] + sensorValues[2] + sensorValues[3] + sensorValues[4] + sensorValues[5] + sensorValues[6] + sensorValues[7] < 4250){ // Mettre la sensibilité en variable ?
    SuiviDligne(vitesseDem);
    positionLigne = qtr.readLineBlack(sensorValues);
    MDROIT.motor.run(FORWARD);     
    MGAUCHE.motor.run(FORWARD);
...
void parcoursAD(){
  ...
  avanceDeMmLigne(280, 1.50); // A REGLER
  ...
  avanceDeMm(30 - MDROIT.TickstoMM(MDROIT.ttlTicks - distempR), 0.70); // avance de Mm Ligne à mettre
  //delay(500);
...
void parcoursDE(){
  ...
  avanceDeMmDecaleLigne(300, 0.75);  //  Décalé de Ligne !!

  distempR = MDROIT.ttlTicks;
  distempL = MGAUCHE.ttlTicks;
  while(((MDROIT.ttlTicks + MGAUCHE.ttlTicks)/2 - (distempR + distempL)/2) < MDROIT.MMtoTicks(50) and (not borne)){
      ToutDroitCapitaine(0.75, MDROIT.ttlTicks - distempR, MGAUCHE.ttlTicks - distempL);
      MDROIT.motor.run(FORWARD);   
      MGAUCHE.motor.run(FORWARD);
      bornemuch(1.5);
  }
  if (borne){
    ...
    avanceDeMmDecaleLigne(50 - (MDROIT.TickstoMM(MDROIT.ttlTicks - distempR)) , 0.75);
    
  }
  
  avanceDeMmDecaleLigne(435, 0.75);   //Décalé de ligne !!

  if (not borne){
    while(((MDROIT.ttlTicks + MGAUCHE.ttlTicks)/2 - (distempR + distempL)/2) < MDROIT.MMtoTicks(100) and (not borne)){
      ToutDroitCapitaine(0.75, MDROIT.ttlTicks - distempR, MGAUCHE.ttlTicks - distempL);
      bornemuch(1.5);
      MDROIT.motor.run(FORWARD);   
      MGAUCHE.motor.run(FORWARD);
    ...
    if (borne){
    avanceDeMmDecaleLigne(100 - (MDROIT.TickstoMM(MDROIT.ttlTicks - distempR)) , 0.75); 
    }
  } else {
    avanceDeMmDecaleLigne(100, 0.75);
  }

  delay(500);
  
  avanceDeMmDecaleLigne(440, 0.75); //Décalé de ligne !!
...
void parcoursDG(){
  ...
  avanceDeMmDecaleLigne(310, 0.75);  //  Décalé de Ligne !!
  ...
  avanceDeMmDecaleLigne(450, 0.75);   //Décalé de ligne !!
  ...
  avanceDeMmDecaleLigne(240, 0.75);
