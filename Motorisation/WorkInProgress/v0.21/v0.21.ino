...
void avanceDeMmInterLigne(float distDem, float vitesseDem){
  
  distempR = MDROIT.ttlTicks;
  distempL = MGAUCHE.ttlTicks;

  while(sensorValues[0] + sensorValues[1] + sensorValues[2] + sensorValues[3] + sensorValues[4] + sensorValues[5] + sensorValues[6] + sensorValues[7] < 7000){
    SuiviDligne(vitesseDem);
  }
  ToutDroitCapitaine(0,0,0);
}
...
void loop() {
  ...
  unsigned int tempos = positionLigne;
  
  avanceDeMmLigne(280, 1.20);
  
  ElVirage(1.00, "d");
  
  while (tempos >= 1000){ // Le robot quitte la ligne
