...
class ClasseMoteur{
  ...
  public:
    ...
    static volatile int _hallTicks;
    double speedM;
    long ttlTicks;
    double cibleVitesse;
    AF_DCMotor _motor;
    ...
    void routine(){
      MotorAsserv();
      speedM = (_hallTicks/_tickperround)/0.02; // Le 0.1 doit correspondre a la fréquence de relevé de speed ligne 20
      ttlTicks += _hallTicks;
      _hallTicks = 0;
    }
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(19), MDROIT.compteur, CHANGE);
  attachInterrupt(digitalPinToInterrupt(18), MGAUCHE.compteur, CHANGE);
